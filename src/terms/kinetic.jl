...
struct TermKinetic{T, AT} <: Term
    scaling_factor::T     # scaling factor, absorbed into kinetic_energies
    kinetic_energies::AT  # kinetic energy 1/2|G+k|^2 for every kpoint
end
function TermKinetic(basis::PlaneWaveBasis{T}, scaling_factor) where {T}
    kinetic_energies = [[T(scaling_factor) * sum(abs2, G + kpt.coordinate_cart) / 2
