...
(P::PowerNonlinearity)(::PlaneWaveBasis{T}) where {T} = TermPowerNonlinearity{T}(P.C, P.α)
...
function compute_kernel(term::TermPowerNonlinearity, ::Basis; ρ, kwargs...)
    Diagonal(vec(_pnl_kernel(term.C, term.α, ρ)))
end

function apply_kernel(term::TermPowerNonlinearity, ::Basis, δρ; ρ, kwargs...)
